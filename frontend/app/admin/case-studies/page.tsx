...
export default function AdminCaseStudiesPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<AdminCaseStudy | null>(null);

  const { data, isLoading, isError } = useQuery({
    queryKey: caseStudyQueryKeys.all,
    queryFn: ({ signal }) => listCaseStudies(signal),
  });

  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: caseStudyQueryKeys.all });

  const statusMutation = useMutation({
    mutationFn: ({ slug, status }: { slug: string; status: ContentStatus }) =>
      setCaseStudyStatus(slug, status),
    onSuccess: (updated) => {
  ...
  const caseStudies = [...(data ?? [])].sort((a, b) => a.sort_order - b.sort_order);
    ...
    <div className="space-y-6">
      ...
      <Panel>
        {isLoading ? (
          ...
          <EmptyState title="Couldn’t load case studies" description="Refresh the page to try again." />
        ) : caseStudies.length === 0 ? (
          <EmptyState
            title="No case studies yet"
            description="Create your first case study to publish it to the website."
          />
        ) : (
            ...
            <table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="border-b border-hairline text-left text-xs font-semibold uppercase tracking-wide text-ink-subtle">
                  <th className="px-5 py-3">Title & Client</th>
              ...
              <tbody className="divide-y divide-hairline">
                {caseStudies.map((caseStudy) => (
                  <tr key={caseStudy.id} className="group transition-colors hover:bg-neutral-50">
                    <td className="px-5 py-3.5">
                      <div className="font-medium text-ink">{caseStudy.title}</div>
                    ...
                    <td className="px-5 py-3.5">
                      {canPublish ? (
                        <Select
                          aria-label={`Status for ${caseStudy.title}`}
                        ...
                        <StatusPill status={caseStudy.status} />
                      )}
                    </td>
                    <td className="px-5 py-3.5">
      ...
      {caseStudy && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <h3 className="mt-4 text-lg font-semibold text-ink">Delete “{caseStudy.title}”?</h3>
            <p className="mt-1.5 text-sm text-ink-muted">
              This removes the case study from the website. This can be undone by an administrator.
            </p>
