...
import { createIndustry, industryQueryKeys, updateIndustry } from '@/lib/admin/industries';
...
export function IndustryForm({ industry }: { industry?: AdminIndustry }) {
  ...
  const {
    ...
    defaultValues: {
      ...
      icon: industry?.icon ?? '',
      sort_order: String(industry?.sort_order ?? 0),
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return isEdit ? updateIndustry(industry!.slug, payload) : createIndustry(payload);
    },
    onSuccess: (saved) => {
      toast.success(isEdit ? 'Changes saved.' : `“${saved.name}” created.`);
      void queryClient.invalidateQueries({ queryKey: industryQueryKeys.all });
    ...
    onError: (error) => {
      if (error instanceof AdminApiError && error.status === 422) {
        ...
        toast.error('Could not save the industry.');
      }
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <PageHeading
          title={isEdit ? `Edit ${industry!.name}` : 'New industry'}
          description={
            isEdit
              ? 'Update the content shown on the industry page.'
              : 'Add a new target sector. It starts as a draft until you publish it.'
          }
        />
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-6" noValidate>
        <Panel className="space-y-5 p-6">
          ...
          <Field
            ...
            hint={`Public URL: /industries/${slugValue || slugify(nameValue) || 'your-industry'}`}
          >
        ...
        <Panel className="space-y-5 p-6">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Icon" htmlFor="icon" error={errors.icon?.message} hint="lucide icon name, e.g. building2">
