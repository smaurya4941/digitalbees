...
export default function RotatingBadge({ 
  ...
  const btnClasses = size === "md" ? "h-12 w-12" : "h-16 w-16 bg-gold-muted text-on-primary";
  ...
  const content = (
    <div className={`relative flex ${sizeClasses} items-center justify-center group cursor-pointer`}>
      <svg className="absolute inset-0 h-full w-full animate-[spin_10s_linear_infinite]" viewBox="0 0 100 100">
        ...
        <text className={`fill-current ${icon === "play" ? "text-on-surface-variant" : "text-white"} ${textClasses} font-bold uppercase tracking-[2px]`}>
          <textPath href="#circlePath" startOffset="0%">{text}</textPath>
        </text>
      </svg>
      <div className={`absolute flex ${btnClasses} items-center justify-center rounded-full bg-gold-muted text-white shadow-lg transition-transform duration-300 group-hover:scale-110`}>
