...
export default function AdminRegionsPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<AdminRegion | null>(null);
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: regionQueryKeys.all });

  const statusMutation = useMutation({
    mutationFn: ({ slug, status }: { slug: string; status: ContentStatus }) =>
      setRegionStatus(slug, status),
    onSuccess: (updated) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (slug: string) => deleteRegion(slug),
    onSuccess: () => {
  ...
  const regions = [...(data ?? [])].sort((a, b) => a.sort_order - b.sort_order);
  ...
  return (
    <div className="space-y-6">
      <PageHeading
        title="Regions"
        description="Geographic areas where services are delivered."
        actions={
          canCreate ? (
            <Link
      ...
      <Panel>
        {isLoading ? (
          ...
          <EmptyState title="Couldn’t load regions" description="Refresh the page to try again." />
        ) : regions.length === 0 ? (
          <EmptyState
            title="No regions yet"
            description="Create your first region to publish it to the website."
          />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-sm">
              ...
              <tbody className="divide-y divide-hairline">
                {regions.map((region) => (
                  <tr key={region.id} className="group transition-colors hover:bg-neutral-50">
                    <td className="px-5 py-3.5">
                      <div className="font-medium text-ink">{region.name}</div>
                      <div className="text-xs text-ink-subtle">/{region.slug}</div>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      {canPublish ? (
                        <Select
                          aria-label={`Status for ${region.name}`}
                          value={region.status}
      ...
      {region && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <h3 className="mt-4 text-lg font-semibold text-ink">Delete “{region.name}”?</h3>
            <p className="mt-1.5 text-sm text-ink-muted">
              This removes the region from the website. This can be undone by an administrator.
            </p>
