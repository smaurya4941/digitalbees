...
export default async function ServicesSection() {
  ...
  const shapePool = ["rounded-full", "rounded-[2rem]", "rounded-full rounded-tr-none", "rounded-full"];
    ...
    <section className="py-24 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto bg-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {practices.map((practice, index) => {
          ...
          return (
          <ScrollReveal key={practice.id} delay={0.1 * (index + 1)}>
            <div className="flex flex-col h-full group cursor-pointer text-left">
              <div className={`w-full aspect-square ${shape} overflow-hidden mb-6 relative transition-transform duration-300 group-hover:-translate-y-2 bg-[#F4EDE4]`}>
                <Image
