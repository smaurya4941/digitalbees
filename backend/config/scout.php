...
return [
    ...
    */
    ...
    // TeamBees: read from SEARCH_DRIVER (see .env.example's "Search" block)
    // with SCOUT_DRIVER as a fallback alias, defaulting to the zero-infra
    // `database` engine (blueprint §30.2's fallback option) rather than
    // Scout's own `collection` default, so search works out of the box in
    // any environment without a Meilisearch server.
    'driver' => env('SEARCH_DRIVER', env('SCOUT_DRIVER', 'database')),
